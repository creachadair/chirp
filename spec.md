...
A Chirp protocol session begins by establishing a reliable, ordered, bidirectional channel carrying packets between a pair of peers.

- **Reliable** means the channel must either deliver each packet sent, or must be marked as closed and report an error. A closed channel is invalid and must report an error for any subsequent use.
- **Ordered** means packets must be delivered to the receiver in the order they were presented by the sender.
- **Bidirectional** means a send and a receive can proceed on a channel concurrently. Sending MUST NOT block receiving and vice versa.
...
**Implementation note:** The bidirectionality requirement rules out a processing loop that presumes strict alternation of sends and receives (with either polarity). The peer can impose limits on how many [calls](#call-subprotocol) may be in flight concurrently, as long as delaying a receive does not prevent sending a response, or vice versa.
...
1. The caller sends a `Request(id, method, params)` packet to the callee. At this point the call is *pending*. The call remains pending until either *terminated* or *completed* according to the rules below.
   ...
   - If `id` duplicates an already pending request, the callee MUST send a `Response(id, DUPLICATE_REQUEST, nil)` packet. This terminates the call. The callee SHOULD also interrupt the already-pending request handler, if possible. Whether or not this succeeds, the callee MUST send a second `Response(id, DUPLICATE_REQUEST, nil)` packet as the response to the already-pending call, thus terminating it, and discard any other result produced by the handler.
